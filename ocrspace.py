...
def ocr_space_image(pil_image, api_key=os.environ.get("OCR_SPACE_API_KEY"), language="eng"):
    """Send a PIL Image to OCR.space API and return extracted text."""
    ...
    # return parsed_results[0].get("ParsedText", "").strip().replace("\n", " ")
    text = parsed_results[0].get("ParsedText", "").strip().replace("\n", " ")
    return " ".join(text.split())  # collapses multiple spaces into one
